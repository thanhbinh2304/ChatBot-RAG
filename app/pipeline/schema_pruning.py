...
CashReceipt(receiptId [PK], receiptDate, receiptAmount, note, customerId [FK -> Customer.customerId], createdBy [FK -> Employee.employeeId], transactionTypeId [FK -> TransactionType.transactionTypeId], invoiceId [FK -> SaleInvoice.invoiceId], PaymentMethod, objectId [FK -> Object.objectId], supplierId [FK -> Supplier.supplierId], receiptCode, createdDate, employeeId) -- Phiếu thu tiền (Tiền vào)
...
Payment(paymentId [PK], paymentDate, paymentAmount, notes, objectId [FK -> Object.objectId], createdBy [FK -> Employee.employeeId], transactionTypeId [FK -> TransactionType.transactionTypeId], purchaseId [FK -> PurchaseOrder.purchaseId], stockId [FK -> Stock.stockId], paymentMethod, supplierId [FK -> Supplier.supplierId], customerId [FK -> Customer.customerId], employeeId, paymentCode) -- Phiếu chi tiền (Tiền ra)
...
PurchaseOrder(purchaseId [PK], purchaseDate, totalAmount, employeeId [FK -> Employee.employeeId], supplierId [FK -> Supplier.supplierId], note, orderType, stockId [FK -> Stock.stockId], discountAmount, paidAmount, purchaseCode) -- Phiếu nhập hàng / Mua hàng từ nhà cung cấp
SaleInvoice(invoiceId [PK], invoiceDate, totalAmount, discountAmount, paidAmount, note, employeeId [FK -> Employee.employeeId], customerId [FK -> Customer.customerId], gasBookId [FK -> GasBook.gasBookId], stockId [FK -> Stock.stockId], orderType, invoiceCode, PaymentMethod) -- Hóa đơn bán hàng / Doanh thu bán hàng
